...
@QuarkusTest
class NumericValuesInStringFieldsFuzzerTest {
    private NumericValuesInStringFieldsFuzzer fuzzer;
    ...
    void setup() {
        ...
        fuzzer = new NumericValuesInStringFieldsFuzzer(simpleExecutor, processingArguments);
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(fuzzer.description()).isNotBlank();
        Assertions.assertThat(fuzzer.description()).containsIgnoringCase("numeric");
    }

    @Test
    void shouldHaveTypes() {
        Assertions.assertThat(fuzzer.getOriginalType()).isEqualTo("string");
        Assertions.assertThat(fuzzer.getFuzzedType()).isEqualTo("numeric");
    }

    @Test
    void shouldReturnNumericValues() {
        Assertions.assertThat(fuzzer.getFuzzedValues("test"))
                .containsExactly(2, 200, 20.0122, new BigDecimal("9899"),
    ...
    void shouldNotRunWhenNoStringFields() {
        ...
        Mockito.when(data.getPayload()).thenReturn("{\"age\": 25, \"count\": 10}");
    ...
    void shouldRunWhenStringFieldPresent() {
        ...
        // 8 numeric values for 1 string field = 8 calls
    ...
    void shouldRunForMultipleStringFields() {
        ...
        // 8 numeric values for 2 string fields = 16 calls
        Mockito.verify(testCaseListener, Mockito.times(16)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }
    ...
    void shouldSkipFieldNotInPayload() {
        ...
        // Only name field should be fuzzed (8 numeric values)
        Mockito.verify(testCaseListener, Mockito.times(8)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }
    ...
    void shouldHandleNestedStringFields() {
        ...
        // 8 numeric values for 1 nested string field = 8 calls
        Mockito.verify(testCaseListener, Mockito.times(8)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }

    @Test
    void shouldExpect4xxWhenNumericCoercionNotAllowed() {
    ...
    @Test
    void shouldExpect2xxWhenNumericCoercionAllowed() {
        Mockito.when(processingArguments.isStrictTypes()).thenReturn(false);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        fuzzer = new NumericValuesInStringFieldsFuzzer(simpleExecutor, processingArguments);
