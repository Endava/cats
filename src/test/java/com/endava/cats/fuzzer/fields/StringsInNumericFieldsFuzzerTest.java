...
class StringsInNumericFieldsFuzzerTest {
    ...
    @BeforeEach
    void setup() {
        stringsInNumericFieldsFuzzer = new StringsInNumericFieldsFuzzer(Mockito.mock(ServiceCaller.class), Mockito.mock(TestCaseListener.class), Mockito.mock(FilesArguments.class));
    }

    @Test
    void shouldGetSchemasThatTheFuzzerWillApplyTo() {
        Assertions.assertThat(stringsInNumericFieldsFuzzer.getSchemaTypesTheFuzzerWillApplyTo()).containsOnly("integer", "number");
    }
    ...
    void shouldGetDescription() {
        Assertions.assertThat(stringsInNumericFieldsFuzzer.description()).isEqualTo("iterate through each Integer (int, long) and Number field (float, double) and send requests having the `fuzz` string value");
    }
