...
@QuarkusTest
class NumericBooleansInBooleanFieldsFuzzerTest {
    private NumericBooleansInBooleanFieldsFuzzer fuzzer;
    ...
    void setup() {
        ...
        fuzzer = new NumericBooleansInBooleanFieldsFuzzer(simpleExecutor, processingArguments);
    }
    ...
    void shouldHaveTypes() {
        ...
        Assertions.assertThat(fuzzer.getFuzzedType()).isEqualTo("numeric boolean");
    }

    @Test
    void shouldReturnNumericBooleanValues() {
        Assertions.assertThat(fuzzer.getFuzzedValues(true)).containsExactly(1, 0, "1", "0");
    }
    ...
    void shouldRunWhenBooleanFieldPresent() {
        ...
        // 4 numeric boolean values (1, 0, "1", "0") for 1 boolean field = 4 calls
    ...
    void shouldRunForMultipleBooleanFields() {
        ...
        // 4 numeric boolean values for 2 boolean fields = 8 calls
    ...
    void shouldSkipFieldNotInPayload() {
        ...
        // Only active field should be fuzzed (4 numeric boolean values)
        Mockito.verify(testCaseListener, Mockito.times(4)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }
    ...
    void shouldHandleNestedBooleanFields() {
        ...
        // 4 numeric boolean values for 1 nested boolean field = 4 calls
        Mockito.verify(testCaseListener, Mockito.times(4)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }

    @Test
    void shouldExpect4xxWhenNumericBooleanCoercionNotAllowed() {
    ...
    @Test
    void shouldExpect2xxWhenNumericBooleanCoercionAllowed() {
        Mockito.when(processingArguments.isStrictTypes()).thenReturn(false);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        fuzzer = new NumericBooleansInBooleanFieldsFuzzer(simpleExecutor, processingArguments);
