...
@QuarkusTest
class NumericStringsInNumericFieldsFuzzerTest {
    private NumericStringsInNumericFieldsFuzzer fuzzer;
    ...
    void setup() {
        ...
        fuzzer = new NumericStringsInNumericFieldsFuzzer(simpleExecutor, processingArguments);
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(fuzzer.description()).isNotBlank();
        Assertions.assertThat(fuzzer.description()).containsIgnoringCase("number");
    }

    @Test
    void shouldHaveTypes() {
        Assertions.assertThat(fuzzer.getOriginalType()).isEqualTo("number");
        Assertions.assertThat(fuzzer.getFuzzedType()).isEqualTo("number string");
    }

    @Test
    void shouldReturnNumericStringValue() {
        Assertions.assertThat(fuzzer.getFuzzedValues(123)).containsExactly("123");
        Assertions.assertThat(fuzzer.getFuzzedValues(99.99)).containsExactly("99.99");
    }
    ...
    void shouldRunWhenIntegerFieldPresent() {
        ...
        // 1 string value for 1 integer field = 1 call
    ...
    void shouldRunWhenNumberFieldPresent() {
        ...
        // 1 string value for 1 number field = 1 call
        Mockito.verify(testCaseListener, Mockito.times(1)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }
    ...
    void shouldRunForMultipleNumericFields() {
        ...
        // 1 string value for 2 numeric fields = 2 calls
    ...
    void shouldSkipFieldNotInPayload() {
        ...
        // Only age field should be fuzzed (1 string value)
        Mockito.verify(testCaseListener, Mockito.times(1)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }
    ...
    void shouldHandleNestedNumericFields() {
        ...
        // 1 string value for 1 nested numeric field = 1 call
        Mockito.verify(testCaseListener, Mockito.times(1)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }

    @Test
    void shouldExpect4xxWhenNumericStringCoercionNotAllowed() {
        ...
        // 1 string value for 1 numeric field = 1 call, expecting 4XX
        Mockito.verify(testCaseListener, Mockito.times(1)).reportResult(
    ...
    @Test
    void shouldExpect2xxWhenNumericStringCoercionAllowed() {
        Mockito.when(processingArguments.isStrictTypes()).thenReturn(false);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        fuzzer = new NumericStringsInNumericFieldsFuzzer(simpleExecutor, processingArguments);
        ...
        // 1 string value for 1 numeric field = 1 call, expecting 2XX
        Mockito.verify(testCaseListener, Mockito.times(1)).reportResult(
                Mockito.any(), Mockito.any(), Mockito.any(), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX_TWOXX), Mockito.anyBoolean(), Mockito.anyBoolean());
    }
}
