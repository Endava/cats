...
class SecurityInjectionFuzzerTest {
    ...
    class NoSqlInjectionFuzzerTest {
        ...
        static Stream<Arguments> noSqlInjectionScenarios() {
            return Stream.of(
                    Arguments.of("Executable operator with MongoDB internals", 200,
                            "{\"$where\": \"function() { return this.password; }\", \"_id\": \"507f1f77bcf86cd799439011\"}",
                            "NoSQL injection", true),
                    Arguments.of("Executable operator with error", 200,
                            "{\"$where\": \"function() { return true; }\", \"error\": \"MongoError: unknown operator\"}",
                            "NoSQL injection", true),
                    Arguments.of("MongoDB internals with error", 200,
                            "{\"objectid\": \"507f1f77bcf86cd799439011\", \"collection\": \"users\", \"error\": \"query failed\"}",
                            "NoSQL injection", true),
                    Arguments.of("Only operator (no JS)", 200,
                            "{\"$where\": \"1==1\"}",
                            "accepted", false),
                    Arguments.of("Only MongoDB internals", 200,
                            "{\"_id\": \"507f1f77bcf86cd799439011\"}",
                            "accepted", false),
                    Arguments.of("Only payload reflection", 200,
                            "{\"query\": \"{\\\"$where\\\": \\\"1==1\\\"}\"}",
                            "accepted", false),
                    Arguments.of("Clean response", 200,
                            "{\"status\": \"ok\"}",
