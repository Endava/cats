...
@QuarkusTest
class DeleteHasBodyLinterFuzzerTest {
    private TestCaseListener testCaseListener;
    private DeleteHasBodyLinterFuzzer deleteHasBodyLinterFuzzer;
    ...
    void setup() {
        ...
        deleteHasBodyLinterFuzzer = new DeleteHasBodyLinterFuzzer(testCaseListener);
    }
    ...
    void shouldReportError() throws Exception {
        ...
        deleteHasBodyLinterFuzzer.fuzz(data);

        Mockito.verify(testCaseListener, Mockito.times(1)).reportResultError(Mockito.any(), Mockito.any(), Mockito.eq("DELETE has body"), Mockito.eq("DELETE method should not have a body"));
    }
    ...
    void shouldReportInfo() throws Exception {
        ...
        Mockito.verify(testCaseListener, Mockito.times(1)).reportResultInfo(Mockito.any(), Mockito.any(), Mockito.eq("DELETE method does not have a body"));
    }

    @Test
    void shouldNotRunWhenNoDelete() throws Exception {
        OpenAPI openAPI = new OpenAPIParser().readContents(Files.readString(Paths.get("src/test/resources/petstore.yml")), null, null).getOpenAPI();
        FuzzingData data = FuzzingData.builder().openApi(openAPI).pathItem(openAPI.getPaths().get("/pets/{id}")).reqSchema(new Schema()).build();
        deleteHasBodyLinterFuzzer.fuzz(data);
    ...
    void shouldReturnSimpleClassNameForToString() {
        Assertions.assertThat(deleteHasBodyLinterFuzzer).hasToString(deleteHasBodyLinterFuzzer.getClass().getSimpleName());
    }

    @Test
    void shouldReturnMeaningfulDescription() {
        Assertions.assertThat(deleteHasBodyLinterFuzzer.description()).isEqualTo("checks if DELETE methods have a body");
    }
}
