...
@QuarkusTest
class ZeroDecimalBodyFuzzerTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private ZeroDecimalBodyFuzzer zeroDecimalBodyFuzzer;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        zeroDecimalBodyFuzzer = new ZeroDecimalBodyFuzzer(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testCaseExporter", Mockito.mock(TestCaseExporter.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        zeroDecimalBodyFuzzer.fuzz(Mockito.mock(FuzzingData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonFuzzer_thenTheResultsAreCorrectlyReported() {
        ...
        zeroDecimalBodyFuzzer.fuzz(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(zeroDecimalBodyFuzzer).hasToString(zeroDecimalBodyFuzzer.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(zeroDecimalBodyFuzzer.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(zeroDecimalBodyFuzzer.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveZeroDecimalPayload() {
        Assertions.assertThat(zeroDecimalBodyFuzzer.getPayload(null)).isEqualTo("0.0");
    }
}
