...
@QuarkusTest
class EmptyJsonBodyFuzzerTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private EmptyJsonBodyFuzzer emptyJsonBodyFuzzer;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        emptyJsonBodyFuzzer = new EmptyJsonBodyFuzzer(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testCaseExporter", Mockito.mock(TestCaseExporter.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        emptyJsonBodyFuzzer.fuzz(Mockito.mock(FuzzingData.class));
    ...
    void shouldHaveToString() {
        Assertions.assertThat(emptyJsonBodyFuzzer).hasToString(emptyJsonBodyFuzzer.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(emptyJsonBodyFuzzer.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(emptyJsonBodyFuzzer.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveNullPayload() {
        Assertions.assertThat(emptyJsonBodyFuzzer.getPayload(null)).isEqualTo("{}");
    }
}
