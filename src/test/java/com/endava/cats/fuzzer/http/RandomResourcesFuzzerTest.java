...
class RandomResourcesFuzzerTest {
    ...
    void shouldNotReplaceUrlParams(HttpMethod method, int times) {
        ...
        Mockito.verify(testCaseListener, Mockito.times(times)).reportResult(Mockito.any(), Mockito.eq(data), Mockito.eq(catsResponse), Mockito.eq(ResponseCodeFamily.FOURXX_NF_AND_VALIDATION), Mockito.anyBoolean(), Mockito.eq(true));
    }
    ...
    void shouldRunForPathsWithVariables(String payload) {
        ...
        Mockito.verify(testCaseListener, Mockito.times(10)).reportResult(Mockito.any(), Mockito.eq(data), Mockito.eq(catsResponse), Mockito.eq(ResponseCodeFamily.FOURXX_NF_AND_VALIDATION), Mockito.anyBoolean(), Mockito.eq(true));
    }
