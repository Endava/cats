...
@QuarkusTest
class RandomNegativeDecimalBodyFuzzerTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private RandomNegativeDecimalBodyFuzzer randomNegativeDecimalBodyFuzzer;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        randomNegativeDecimalBodyFuzzer = new RandomNegativeDecimalBodyFuzzer(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        randomNegativeDecimalBodyFuzzer.fuzz(Mockito.mock(FuzzingData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonFuzzer_thenTheResultsAreCorrectlyReported() {
        ...
        randomNegativeDecimalBodyFuzzer.fuzz(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(randomNegativeDecimalBodyFuzzer).hasToString(randomNegativeDecimalBodyFuzzer.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(randomNegativeDecimalBodyFuzzer.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(randomNegativeDecimalBodyFuzzer.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveNegativeDecimalPayload() {
        Assertions.assertThat(randomNegativeDecimalBodyFuzzer.getPayload(null)).isEqualTo("-9.876987698769877E11");
    }
}
