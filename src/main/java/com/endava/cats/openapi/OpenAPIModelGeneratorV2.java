...
public class OpenAPIModelGeneratorV2 {
    ...
    public List<String> generate(String modelName) {
        ...
        if (modelName != null) {
            ...
            if (schema != null) {
                List<Map<String, Object>> generatedExamples = generateExamplesForSchema(null, schema);
    ...
    private List<Map<String, Object>> resolveAllOfSchemaProperties(Schema schema, String propertyName) {
        ...
        for (Schema subSchema : schemas) {
            ...
            if (ModelUtils.isAnyOf(subSchema) || ModelUtils.isOneOf(subSchema)) {
    ...
    private List<Map<String, Object>> resolveAnyOfOneOfSchemaProperties(String propertyName, Schema schema) {
        ...
        List<Map<String, Object>> examples = new ArrayList<>();
        List<Schema> schemas = ModelUtils.getInterfaces(schema).stream().filter(iteratingSchema -> !isNullSchema(iteratingSchema)).toList();
    ...
    private List<Object> resolvePropertyToExamples(String propertyName, Schema property, String currentSchemaName) {
        ...
        if (ModelUtils.isArraySchema(property)) {
            Schema itemSchema = ModelUtils.getSchemaItems(property);
            ...
            Schema additionalProperties = ModelUtils.getAdditionalProperties(property);
        ...
        } else if (ModelUtils.isAllOf(property) || ModelUtils.isAllOfWithProperties(property)) {
            examples.addAll(resolveAllOfSchemaProperties(property, propertyName));
        } else if (ModelUtils.isOneOf(property) || ModelUtils.isAnyOf(property)) {
    ...
    private Object matchToEnumOrEmpty(String name, Schema innerSchema, String propertyName) {
        ...
        String result = Optional.ofNullable(resolveInnerSchema.getEnum()).orElse(List.of("")).stream().filter(value -> name.contains(value.toString())).findFirst().orElse("").toString();
        if (result.isEmpty()) {
